...
"""
This data util is useful since when training a model, we are actually using the same pre-processed set of data based on
the data split produced.

This data util simply reads all the raw training data from <root_dir>/data_split.pkl file, pre-process it, and store it
in another pickle file in <root_dir>/edwin/data.

During training, the training script only needs to read the pickle file generated by this utility script. Thus, the
training process becomes much faster.
"""


X_SET_TO_BE_PREPROCESSED = 'X_AB'
Y_SET_TO_BE_PREPROCESSED = 'Y_AB'
...
def read_and_combine_training_data():
    ...
    result_df = train_data_split_df.merge(users_df, left_on='User', right_on='RESPID', how='inner')
    result_df = result_df.merge(words_df, left_on=['Artist', 'User'], right_on=['Artist', 'User'], how='inner')
...
def combine_testing_data(test_split_df, training_mean_std_per_column):
    ...
    result_df = test_split_df.merge(users_df, left_on='User', right_on='RESPID', how='inner')
    result_df = result_df.merge(words_df, left_on=['Artist', 'User'], right_on=['Artist', 'User'], how='inner')
    result_df = drop_irrelevant_cols_from_merged_df(result_df)
    return result_df
...
def get_train_data_split_df():
    ...
    X_train_data_split = data_split[X_SET_TO_BE_PREPROCESSED]
    Y_train_data_split = data_split[Y_SET_TO_BE_PREPROCESSED]
...
if __name__ == '__main__':
    ...
    # Store the result in a zipped pickle file
